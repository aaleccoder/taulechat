...
export class OpenRouterProvider implements ChatProvider {
    ...
    return messages.map((m: any) => {
      ...
      if (m.role === "user" && (m.files?.length || 0) > 0) {
        ...
        for (const f of m.files!.slice(0, 2)) {
          // OpenRouter supports max 2 files
          if (f.mime_type.startsWith("image/") && options?.supportsImages) {
            ...
            const hasAnnotations = this.checkForExistingAnnotations(
              messages,
              f.file_name,
            );
  ...
  private checkForExistingAnnotations(
    messages: any[],
    filename: string,
  ): boolean {
  ...
  async streamResponse(
    ...
    const hasPdfFiles = request.messages.some(
      (msg: any) =>
        Array.isArray(msg.content) &&
        msg.content.some(
          (part: any) =>
            part.type === "file" &&
            part.file?.file_data?.includes("application/pdf"),
        ),
    );
    ...
    if (request.parameters) {
      ...
      if (params.reasoningLevel) {
        ...
        requestBody.reasoning.effort =
          reasoningLevels[params.reasoningLevel] || "medium";
      }
      ...
      if (params.transforms !== undefined)
        requestBody.transforms = params.transforms;
      ...
      if (params.logit_bias !== undefined)
        requestBody.logit_bias = params.logit_bias;
      if (params.top_logprobs !== undefined)
        requestBody.top_logprobs = params.top_logprobs;
      if (params.prediction !== undefined)
        requestBody.prediction = params.prediction;
    }
    ...
    if (!response.body)
      throw new ProviderResponseError(
        "OpenRouter response missing body stream",
      );
      ...
      try {
        ...
        if (parsed.usage) {
          usageMetadata = {
            ...
            imageTokenCount:
              parsed.usage.completion_tokens_details?.image_tokens || 0,
            cachedTokenCount:
              parsed.usage.prompt_tokens_details?.cached_tokens || 0,
            audioTokenCount:
              parsed.usage.prompt_tokens_details?.audio_tokens || 0,
            cost: parsed.usage.cost || 0,
            upstreamCost:
              parsed.usage.cost_details?.upstream_inference_cost || 0,
            upstreamPromptCost:
              parsed.usage.cost_details?.upstream_inference_prompt_cost || 0,
            upstreamCompletionsCost:
              parsed.usage.cost_details?.upstream_inference_completions_cost ||
              0,
        ...
        if (delta?.images && Array.isArray(delta.images)) {
          for (const img of delta.images) {
            if (img.type === "image_url" && img.image_url?.url) {
              images.push({
                mimeType:
                  img.image_url.url.split(";")[0].split(":")[1] || "image/png",
