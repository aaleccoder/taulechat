...
export default function LinkPreviewTooltip({
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col gap-2 p-4 max-w-[90vw] sm:max-w-[40vh]">
            {meta.title && (
              <div className="font-semibold truncate text-sm">{meta.title}</div>
            )}
            {meta.description && (
            ...
            <div
              className="text-xs text-accent-foreground break-all cursor-pointer hover:text-accent-foreground underline touch-manipulation min-h-[44px] flex items-center px-3 py-2 -mx-3 rounded motion-safe:transition-all motion-safe:duration-150 active:scale-95 active:bg-accent/20"
            ...
            {meta.ogImage && (
              <img
                ...
                className="mt-2 max-w-full max-h-48 w-auto h-auto object-cover rounded shadow-sm"
              />
            )}
          </CardContent>
        </Card>
  ...
  return (
    <Tooltip onOpenChange={handleTooltipOpenChange}>
      ...
      <TooltipContent
        className={`${loading ? "bg-card text-white" : "bg-transparent"}`}
      >
        {previewContent}
      </TooltipContent>
    </Tooltip>
