...
const AssistantMessage = memo(function AssistantMessage({
  ...
  const { cancelStream } = useStreamingStore();
  let renderedContent = message.content;
  if (
    message.groundingSupports &&
    Array.isArray(message.groundingSupports) &&
    ...
    const supportsSorted = [...message.groundingSupports].sort(
      (a, b) => b.segment.endIndex - a.segment.endIndex,
    );
    supportsSorted.forEach((support) => {
      const indices = (support.groundingChunkIndices as number[]).map(
        (i: number) => i + 1,
      );
      if (indices.length > 0) {
        const uniqueIndices = [...new Set(indices)];
        const chunk = uniqueIndices
          .map((index) => message.groundingChunks[index - 1])
        ...
        if (chunk?.web?.uri) {
          ...
          const existingLink = afterText.match(
            /^\s*(\[.*?\]\(.*?\)|https?:\/\/[^\s]+)/,
          );
          if (!existingLink) {
    ...
    >
      ...
      {hasThoughts && (
        ...
        >
          ...
          <CollapsibleContent
            className="mt-2 rounded-lg border bg-background px-3 py-2 shadow-md text-muted-foreground text-sm whitespace-pre-wrap overflow-hidden blind-animate"
            aria-label="Assistant thought process"
          >
      ...
      {loading && !message.content ? (
        ...
        <div className="relative">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="absolute -top-2 -right-2 h-6 w-6 p-0 opacity-50 hover:opacity-100 bg-background border shadow-sm"
