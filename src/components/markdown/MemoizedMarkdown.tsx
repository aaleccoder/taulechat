...
const Anchor: FC<ComponentProps<"a">> = ({ href, children }) => {
  ...
  return (
    <LinkPreviewTooltip href={href}>
      <span className="cursor-pointer !no-underline bg-accent/90 text-accent-foreground px-2 rounded-full py-1 text-xs hover:bg-accent/10 transition">
        {children}
      </span>
    </LinkPreviewTooltip>
  );
...
const createCodeBlock =
  (onCopy: (text: string) => void): FC<ComponentProps<"code">> =>
  (props) => {
